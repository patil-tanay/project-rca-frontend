...
function Task() {
    ...
    useEffect(() => {
        if (!isAddTaskModalOpen || isRenderChange) {
            axios.get(`http://13.239.35.37:9000/project/${projectId}`)
                .then((res) => {
    ...
    const updateTodo = (data) => {
        axios.put(`http://13.239.35.37:9000/project/${projectId}/todo`, data)
            .then((res) => {
            }).catch((error) => {
                toast.error('Something went wrong')
            })
    ...
    const handleDelete = (e, taskId) => {
        e.stopPropagation();
        axios.delete(`http://13.239.35.37:9000/project/${projectId}/task/${taskId}`)
            .then((res) => {
